...
class lib_clase:
    ...
    def date_features(date):
        """
        Function to obtain cyclical features from a date
        Input: date in datetime format, or string that can be used with pd.to_datetime
        Example: '%Y-%m-%d', '2021-11-22'
        
        Returns
        -------
        Cyclycal date features
        month_x: x component of month of given date
        month_y: y component of month of given date
        day_x: x component of day of given date
        day_y: y component of day of given date
        """
        
        try:
            date = pd.to_datetime(date)
        except:
            print("Date format cannot be converted to datetime using pd.to_datetime, fix date format.")
        
        #The cyclical function is 2pi/T * t, where T is the period
        month_x = np.sin( 2 * np.pi * date.month / 12 )
        month_y = np.cos( 2 * np.pi * date.month / 12 )
        
        #We obtain the numer of days in the month
        _,num_days = monthrange(date.year, date.month)
        
        day_x = np.sin( 2 * np.pi * date.day / num_days )
        day_y = np.cos( 2 * np.pi * date.day / num_days )
        
            
        return [month_x, month_y, day_x, day_y]
        
        
    @staticmethod      
    def time_features(thetime):
        """
        Function to obtain cyclical features from time
        Input: time in the datetime format, or string that can be used with pd.to_datetime
        Example: '13:01:13'
        
        Returns
        -------
        Cyclycal hour features
        hour_x: x component of hour of given time
        hour_y: y component of month of given time
        """
    
        
        thetime = "13:01:13"        
        thetime = pd.to_datetime(thetime)
    
        hourtime = thetime.hour + thetime.minute/60 + thetime.minute/3600
        
        hour_x = np.sin( 2 * np.pi * hourtime / 24 )
        hour_y = np.cos( 2 * np.pi * hourtime / 24 )
    ...
    def MAD(x):
        ...
        Returns:
            Median Absolute Deviation
        """
        B=stats.median_abs_deviation(x,scale='normal')
        return B
...
#%% datsets
    ...
    def load_data_slope():       
        print('1. Cargando Data')
        filename = 'data\\dataslope.csv'        
...
#%% Funciones para hacer SQL querys 
    ...
    def sqlquery_tbv(self, fecha_inicial, fecha_final, dictio_credenciales = 'env_dictio.json'):
        """Funcion para correr un SQL query y obtener el numero de casinos distintos por cliente dentro de cierto periodo.
        ...
        sqlquery = """
                select player_id, Casino,
                    CONVERT(varchar,Gamingdate,112) as Jornada,
                    DATEDIFF(day, LAG(Gamingdate) OVER (PARTITION BY player_id ORDER BY Gamingdate), Gamingdate )  as tbv
                    from imercado..tafd_resumen
                    where Casino <> 'Mendoza'
                    and (Slot > 0 or Mesa > 0 or Bingo > 0)
                    and Gamingdate between '%s' and '%s'
                    and player_id in (
                    			SELECT  distinct ID_MCC
                    			FROM [imercado].[dbo].[DM_BSE_CLIENTES]
                                where CARTERA_VIGENTE = 'S')
